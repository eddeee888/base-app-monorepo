...
import { canUserViewUserPrivateDetails } from "graphql/permissions";
    ...
    field.resolve = async function (...params) {
      ...
      switch (parentType.name) {
        case AcceptableParentType.User:
          ...
          break;
        default:
          // TODO: log an error here to report that isPrivate is used on an object that is not valid
          console.log(`@isPrivate is used on an unsupported type "${parentType.name}"`);
          break;
      }
