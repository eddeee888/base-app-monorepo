        ...
        <>
          {defaultProps.values.sessions.map((session, index) => (
            <div key={index}>
              ...
              <button onClick={duplicateSessionFns[index]}>
                duplicate {index}
              </button>
            </div>
          ))}
...
describe('<SessionBlocksLogic />', () => {
  ...
  it('should call function to remove index of array when removeSession is called', () => {
    ...
    const removeButtonOnClick1 = findButton(wrapper, 'remove 0').prop(
      'onClick'
    );
    ...
    const removeButtonOnClick2 = findButton(wrapper, 'remove 1').prop(
      'onClick'
    );
  ...
  it('should call function to duplicate index of array when duplicateSession is called', () => {
    ...
    const duplicateButtonOnClick1 = findButton(wrapper, 'duplicate 0').prop(
      'onClick'
    );
    (duplicateButtonOnClick1 as any)();
    expect(defaultProps.arrayHelpers.insert).toHaveBeenCalledTimes(1);
    expect(defaultProps.arrayHelpers.insert).toHaveBeenCalledWith(
      1,
      defaultProps.values.sessions[0]
    );

    (defaultProps.arrayHelpers.insert as jest.Mock).mockReset();

    const duplicateButtonOnClick2 = findButton(wrapper, 'duplicate 1').prop(
      'onClick'
    );
    ...
    expect(defaultProps.arrayHelpers.insert).toHaveBeenCalledWith(
      2,
      defaultProps.values.sessions[1]
    );
  });
