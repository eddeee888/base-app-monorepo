...
> {
  ...
  constructor(
    ...
    super(fragments, rawConfig, {
      ...
      hooksImportFrom: getConfigValue(
        rawConfig.hooksImportFrom,
        'react-apollo-hooks'
      ),
      reactApolloImportFrom: getConfigValue(
        rawConfig.reactApolloImportFrom,
        'react-apollo'
      ),
  ...
  private getReactApolloImport(): string {
    return `import * as ReactApollo from '${
      typeof this.config.reactApolloImportFrom === 'string'
        ? this.config.reactApolloImportFrom
        : 'react-apollo'
    }';`;
  }

  private getReactApolloHooksImport(): string {
    return `import * as ReactApolloHooks from '${
      typeof this.config.hooksImportFrom === 'string'
        ? this.config.hooksImportFrom
        : 'react-apollo-hooks'
    }';`;
  }
  ...
  private _buildHocProps(operationName: string, operationType: string): string {
    const typeVariableName = this.convertName(
      operationName + toPascalCase(operationType)
    );
    const variablesVarName = this.convertName(
      operationName + toPascalCase(operationType) + 'Variables'
    );
  ...
  private _buildOperationHoc(
    ...
    const propsTypeName: string = this.convertName(node.name.value, {
      suffix: 'Props'
    });
  ...
  ${propsTypeName}<TChildProps>>) {
    return ReactApollo.with${titleCase(
      node.operation
    )}<TProps, ${operationResultType}, ${operationVariablesTypes}, ${propsTypeName}<TChildProps>>(${documentVariableName}, {
  ...
  private _buildComponent(
    ...
    const componentName: string = this.convertName(node.name.value, {
      suffix: operationType + this.config.componentSuffix,
      useTypesPrefix: false
    });
  ...
  private _buildHooks(
    ...
    return `
export function use${operationName}(baseOptions?: ReactApolloHooks.${operationType}HookOptions<${
      this.config.hooksImportFrom === '@apollo/react-hooks' ||
  ...
  protected buildOperation(
    ...
    return [mutationFn, component, hooks, resultType, mutationOptionsType]
      .filter(a => a)
      .join('\n');
  }
}
