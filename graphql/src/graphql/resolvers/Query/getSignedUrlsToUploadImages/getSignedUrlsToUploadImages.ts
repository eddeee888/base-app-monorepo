...
const getSignedUrlsToUploadImages: QueryResolvers['getSignedUrlsToUploadImages'] = async (
  parent,
  { filenames }
) => {
  ...
  const uploadedFilePromises = Promise.all(
    filenames.map(filename => getSignedUrl(signUrl, filename))
  );
